...
def main():
    ...
    if not load_raw_admin_boundaries(pg_cur, settings):
        logger.error("Job Terminated")
...
def set_arguments():
    ...
    # Work file to correct invalid characters within the GNAF CSV file 
    parser.add_argument(
        '--work-file-prefix',
        default="out_",
        help="Work File prefix required by CSV reader to correct invalid characters found within the gnaf CSV file")
...
def get_settings(args):
    ...
    # work file
    settings['work_file_prefix'] = args.work_file_prefix
...
def populate_raw_gnaf(settings):
    ...
    if len(list) == 0:
        ...
        psma.load_csv_files(files, settings, logger)
        ...
        for file_name in files:
            if file_name.lower().startswith(prefix + "_"):
                if file_name.lower().endswith(".psv"):
                    ...
                    gnaf_files.append({ 'raw_gnaf_schema' : settings['raw_gnaf_schema'], 'table' : table, 'file_path' : file_path })
